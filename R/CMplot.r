...
{   
    ...
    read_annotation_table <- function(annotation.file, sep=NULL) {
        ...
        if(is.null(sep)){
            ext <- tolower(tools::file_ext(annotation.file))
            sep <- ifelse(ext == "csv", ",", "\t")
        }
    ...
    resolve_annotation_col <- function(dat, requested=NULL, candidates=NULL, required=TRUE, fallback=NULL) {
        nms <- names(dat)
    ...
    annotation_to_highlight <- function(annotation.file, trait, R, sep=NULL, snp.col=NULL, label.col=NULL, trait.col=NULL) {
        anno <- read_annotation_table(annotation.file, sep=sep)
        snp.idx <- resolve_annotation_col(anno, snp.col,
            candidates=c("SNP", "SNPID", "Marker", "MarkerName", "ID", "rsID", "variant", "variant_id"),
            required=TRUE)
        label.idx <- resolve_annotation_col(anno, label.col,
            candidates=c("Label", "label", "Gene", "gene", "GeneName", "Annotation", "annotation", "Name", "name"),
            required=FALSE, fallback=snp.idx)
        trait.idx <- resolve_annotation_col(anno, trait.col,
    ...
    if(length(plot.type) > 1 | (!"d" %in% plot.type)){
        ...
        if(!is.null(annotation.file)){
            ...
            annotation.input <- annotation_to_highlight(annotation.file, trait=trait, R=R,
        ...
        if(!is.null(highlight)){
            ...
            for(i in 1:length(highlight)){
                if(sum(!is.na(highlight[[i]])) == 0 | length(highlight[[i]]) == 0){
                    ...
                    highlight_index[[i]] <- match(as.character(unlist(highlight[[i]], use.names=FALSE)), SNP_id)
        ...
        chr.max.pos <- vapply(pvalue.pos.list, max_no_na, numeric(1))
        chr.min.pos <- vapply(pvalue.pos.list, min_no_na, numeric(1))

        #scale the space parameter between chromosomes
        if(!missing(band)){
            band <- floor(band*(sum(chr.max.pos) - min_no_na(chr.min.pos))/100)
        }else{
            band <- floor((sum(chr.max.pos) - min_no_na(chr.min.pos))/100)
        }
        if(band==0) band=100
        ...
        draw_rect_chr_axis <- function(scale=1, Min=NULL, Max=NULL, title=TRUE){
            axis.at <- c(min_no_na(pvalue.posN)-band, ticks)
            ...
            axis(1, mgp=c(3,xticks.pos,0), at=c(ticks[length(ticks)], max_no_na(pvalue.posN)),
        ...
        if(multracks | multraits){
            ...
            if(multraits){
                ...
                if(is.null(ylim)){
                    ...
                    Pmap.min_no_na <- as.numeric(as.matrix(Pmap[,3:(R+2)]))
